    ...
    >
      <div className="flex-1 flex flex-col p-6 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
          <h2 className="text-3xl font-black mb-1 uppercase tracking-tight leading-none text-black">{activity.title}</h2>
          
          <div className="flex items-center gap-2 text-sm font-bold border-b-4 border-black pb-4 mb-4 mt-2 text-black">
            <MapPin size={16} />
          ...
          <div>
            ...
            <p className="text-xl font-bold leading-snug pb-4 text-black">{activity.details}</p>
          </div>
        </div>
      </div>
    </motion.div>
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-8 opacity-50">
        <div className="w-24 h-24 border-4 border-black rounded-full flex items-center justify-center mb-4 bg-gray-100">
          <MessageCircle size={40} strokeWidth={1.5} className="text-black" />
        </div>
        <h2 className="text-xl font-bold mb-2 text-black">No Chats Yet</h2>
        <p className="font-medium text-black">Swipe right on activities to join their group chats!</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full bg-yellow-50">
      <div className="p-6 border-b-4 border-black bg-white sticky top-0 z-10">
        <h1 className="text-3xl font-black uppercase tracking-tighter text-black">Your Groups</h1>
      </div>
        ...
        {chats.map((chat) => (
          ...
          >
            <div className={`w-14 h-14 ${chat.color} rounded-full border-2 border-black shrink-0 flex items-center justify-center`}>
              <chat.Icon size={24} strokeWidth={2} className="text-black" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-baseline mb-1">
                <h3 className="font-bold text-lg truncate text-black">{chat.title}</h3>
                <span className="text-xs font-bold text-black">2m</span>
              </div>
              <p className="text-sm font-medium text-black truncate">Bob: Sounds good to me!</p>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="flex flex-col h-full bg-white animate-in slide-in-from-right duration-300 z-50">
      ...
      <div className="p-4 border-b-4 border-black flex items-center gap-3 bg-white z-10 shadow-sm">
        ...
        <div className="flex-1 min-w-0">
          <h2 className="font-black text-lg truncate leading-none text-black">{activity.title}</h2>
  ...
  <div className="flex flex-col h-full bg-white">
    <div className="p-8 flex flex-col items-center border-b-4 border-black bg-purple-50">
      <div className="w-32 h-32 bg-white border-4 border-black rounded-full mb-4 flex items-center justify-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
        <User size={64} strokeWidth={1.5} className="text-black" />
      </div>
      <h1 className="text-3xl font-black uppercase text-black">Jane Doe</h1>
      <p className="text-lg font-medium text-center text-black">Adventurer. Coffee lover. Night owl.</p>
    </div>
    
    <div className="p-6 space-y-4">
      <div className="text-sm font-bold text-black uppercase tracking-widest mb-2">Account</div>
      
      <button className="w-full bg-white border-2 border-black p-4 rounded-xl font-bold flex justify-between items-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-[2px] active:translate-y-[2px] transition-all text-black">
      ...
      <div className="mt-8 p-6 border-4 border-dashed border-black rounded-xl text-center text-black font-bold text-sm">
...
export default function ActivityTinder() {
             ...
             >
               ...
               <div className="flex-1 relative p-4 pb-0 flex items-center justify-center">
                   ...
                   <div className="absolute top-4 left-6 right-6 z-0 flex justify-between items-center pointer-events-none">
                     <h1 className="text-4xl font-black italic tracking-tighter text-black">FINDER</h1>
                   </div>
                   ...
                   {activityQueue.length > 0 ? (
                     ...
                     <div className="text-center p-8">
                       ...
                       <h2 className="text-2xl font-black mb-2 text-black">You&apos;re caught up!</h2>
                       <p className="mb-6 font-medium text-black">You&apos;ve joined all the available activities in your area.</p>
                       <button 
                        onClick={() => {
        ...
        {!activeChatId && (
          <nav className="h-20 border-t-4 border-black flex justify-around items-center bg-white z-20 pb-2 shadow-[0_-4px_10px_rgba(0,0,0,0.05)] shrink-0">
            <button 
              ...
              className={`flex flex-col items-center p-2 transition-all active:scale-95 text-black ${activeTab === 'activities' ? 'opacity-100' : 'opacity-40 hover:opacity-70'}`}
            >
            ...
            <button 
              ...
              className={`flex flex-col items-center p-2 transition-all active:scale-95 text-black ${activeTab === 'chats' ? 'opacity-100' : 'opacity-40 hover:opacity-70'}`}
            >
            ...
            <button 
              ...
              className={`flex flex-col items-center p-2 transition-all active:scale-95 text-black ${activeTab === 'profile' ? 'opacity-100' : 'opacity-40 hover:opacity-70'}`}
            >
